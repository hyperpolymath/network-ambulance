...
|**Logging System**
|~180
|Multi-level logging (DEBUG→FATAL), file output, colored console, section headers, timestamps
...
* `src/diagnostics/wireless.sh` — new module
...
* `src/diagnostics/vpn.sh` — new module
...
* `src/repairs/firewall.sh` — new module
...
[✓] Firewall repaired. HTTP/HTTPS now accessible.
...
* `src/diagnostics/captive-portal.sh` — new module
...
[✓] Authentication successful! Internet now accessible.
----
...
* `src/diagnostics/proxy.sh` — new module
...
[✗] Cannot reach proxy.corp.com:8080
...
* `docs/PLUGIN_API.adoc` — plugin development guide
...
tests/
├── unit/
│   ├── test_dns_diagnostics.bats
│   ├── test_interface_diagnostics.bats
│   └── ...
├── integration/
│   ├── test_full_repair.bats
│   ├── test_vpn_scenarios.bats
│   └── ...
├── mocks/
│   ├── ip.sh
│   ├── ping.sh
...
|• Firewall diagnostic enhancements
...
|**M2: Firewall Repairs**
|• UFW repair module
 • firewalld repair module
 • iptables repair (safe mode)
...
|• Enhanced systemd-networkd support
 ...
 • Guided NM→systemd-networkd migration
|2 weeks
|Week 13-14 (Jun)
...
* `shellcheck` — linting (already used)
* `jq` — JSON handling for plugin manifests

==== Internal (AmbientOps Integration)

* `system-tools-contracts` — evidence envelope format
* `operating-room` — procedure orchestration
* `emergency-button` — panic mode integration
...
* Follows Scan → Plan → Apply → Receipt workflow
...
* `network-repair status --format weather` — outputs simple status
...
* Contribute code: See link:.github/CONTRIBUTING.md[.github/CONTRIBUTING.md]
...
This roadmap is part of Network Ambulance and is licensed under MPL-2.0.
