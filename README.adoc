...
[quote]
____
"It's 2am, your internet is down, and you have a deadline. You don't need a networking degree—you need Network Ambulance."
____
...
Network Ambulance provides *systematic, expert-level diagnostics and repairs* for common Linux connectivity failures—without requiring you to be a networking expert.
...
When Network Ambulance finds problems, it can fix them—*safely*:
...
[source,text]
----
[✓] Interface enp0s31f6: UP, no IP address (PROBLEM)
[✓] Interface wlp3s0: UP, 10.45.253.69/24
[✗] DNS: No working nameservers in /etc/resolv.conf
[✓] Default route via 10.45.253.205
[✗] Cannot reach 8.8.8.8 (PROBLEM)
[✗] DNS resolution failing for google.com
...
This is the most common problem—you're connected to WiFi, but browsers say "no internet".

[source,bash]
----
$ network-repair diagnose connectivity

[✓] Interface wlp3s0: UP, connected to "Home WiFi"
[✗] Cannot ping 8.8.8.8
...
[✓] DNS repaired. You should have internet now.
----
...
[✗] Interface enp0s31f6: DOWN (no carrier)
...
[✓] Interface enp0s31f6 is now UP with IP 192.168.1.105
----
...
[✗] Multiple default routes detected (conflict)
[✗] Default route points to unreachable gateway 10.8.0.1
...
[✗] NetworkManager cannot manage wlp3s0 (blocked by systemd-networkd)
...
* **Operating Room** — Planned procedures (Network Ambulance lives here)
...
├── network-config/       # Config files (sanitized)
│   ├── resolv.conf
│   ├── NetworkManager.conf
│   └── interfaces
├── logs/                 # Relevant logs
...
* ✓ NetworkManager (primary, fully supported)
* ✓ systemd-networkd (supported)
* ⚠ netplan (via NetworkManager/systemd-networkd)
* ✗ ifupdown (legacy, limited support)
...
└── src/utils/               # Core utilities
    ...
    ├── privileges.sh        # Root/sudo handling
...
Network Ambulance is licensed under MPL-2.0 (Palimpsest Meta-Permissive License).
